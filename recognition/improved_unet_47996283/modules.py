...
class UNet(nn.Module):
    ...
    """
    def __init__(self, in_channels=1, out_channels=4, base_filters=32, dropout_p=0):
        super().__init__()
    ...
    def _initialize_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                # Better initialization for LeakyReLU
    ...
    def _conv_block(self, in_ch, out_ch, dropout_p=0):
...
class CombinedLoss(nn.Module):
    ...
    def __init__(self, num_classes, class_weights, dice_weight=0.5, ce_weight=0.5, smooth=1e-6):
        super().__init__()
